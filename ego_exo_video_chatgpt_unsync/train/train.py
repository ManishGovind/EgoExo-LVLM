...
from ego_exo_video_chatgpt_unsync.train.llava_trainer import VideoChatGPTTrainer
from ego_exo_video_chatgpt_unsync import video_conversation as conversation_lib
from ego_exo_video_chatgpt_unsync.model import *
import torch.distributed as dist
from ego_exo_video_chatgpt_unsync.constants import *
import pickle
...
def preprocess_multimodal_for_ego(
        sources: Sequence[str],
        multimodal_cfg: dict,
        cur_token_len: int,
) -> Dict:
    ...
    for source in sources:
        if multimodal_cfg['sep_video_conv_front']:
            assert DEFAULT_EGO_TOKEN in source[0]['value']
            source[0]['value'] = source[0]['value'].replace(DEFAULT_EGO_TOKEN, '').strip()
            source[0]['value'] = DEFAULT_EGO_TOKEN + conversation_lib.default_conversation.sep + \
                                 conversation_lib.default_conversation.roles[0] + ": " + source[0]['value']
        for sentence in source:
            replace_token = DEFAULT_EGO_PATCH_TOKEN * video_token_len
            if multimodal_cfg['use_vid_start_end']:
                replace_token = DEFAULT_EGO_START_TOKEN + replace_token + DEFAULT_EGO_END_TOKEN 
            if isinstance(sentence["value"], int):
                sentence["value"] = str(sentence["value"])  
            # print(sentence["value"])       
            sentence["value"] = sentence["value"].replace(DEFAULT_EGO_TOKEN, replace_token)
            # print(sentence["value"])
    return sources    
...
class LazySupervisedDataset(Dataset):
    """Dataset for supervised fine-tuning with support for ego and exo data."""
    ...
    def __init__(self, data_path: str,
        ...
        self.ego_list_data_dict = ego_list_data_dict
        self.multimodal_cfg = multimodal_cfg
        print(len(list_data_dict) , len(ego_list_data_dict))
        # # Ensure both datasets have the same length by balancing
        # self.max_len = max(len(self.list_data_dict), len(self.ego_list_data_dict))
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'video' in sources[0]:
            ...
            with open(f"{video_folder}/{exo_video_file}", "rb") as f:
                exo_features = pickle.load(f)
            # print(exo_features.shape)
            cur_token_len = 356  # Adjust token length if needed
            sources = preprocess_multimodal(
                copy.deepcopy([e["conversations"] for e in sources]),
                self.multimodal_cfg, cur_token_len)

        # Process ego video
        if 'video' in ego_sources[0]:
            ego_video_file = self.ego_list_data_dict[ego_idx]['video']
            video_folder = self.multimodal_cfg['video_folder']
        ...
        data_dict_exo = preprocess(sources, self.tokenizer)
        data_dict_ego = preprocess(ego_sources, self.tokenizer)

        # Handle single sample
        if isinstance(i, int):
            data_dict_exo = dict(input_ids=data_dict_exo["input_ids"][0],
                                 labels=data_dict_exo["labels"][0])
            data_dict_ego = dict(input_ids=data_dict_ego["input_ids"][0],
                                 labels=data_dict_ego["labels"][0])

        # Combine ego and exo features in the returned dictionary
        data_dict_exo["video"] = exo_features if 'video' in self.list_data_dict[exo_idx] else None
        data_dict_ego["video"] = ego_features if 'video' in self.ego_list_data_dict[ego_idx] else None

        return data_dict_exo, data_dict_ego
...
#         ego_sources = self.ego_list_data_dict[i]  
#         if isinstance(i, int):
#             sources = [sources]
#         assert len(sources) == 1, "Don't know why it is wrapped to a list"  # F
#         if 'video' in sources[0]:
#             video_file = self.list_data_dict[i]['video']
#             video_folder = self.multimodal_cfg['video_folder']
#             # print(video_folder)
#             # elif "video" in video_file:
#             #     video_folder = '/home/data/NTU_combination_video_features'
#                 # print(video_folder)
...
class DataCollatorForSupervisedDataset(object):
    """Collate examples for supervised fine-tuning with ego and exo videos."""

    tokenizer: transformers.PreTrainedTokenizer

    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels = [], []
        ego_input_ids, ego_labels = [], []
        features, ego_features = [], []

        for instance_exo, instance_ego in instances:
            input_ids.append(instance_exo['input_ids'])
            labels.append(instance_exo['labels'])
            ego_input_ids.append(instance_ego['input_ids'])
            ego_labels.append(instance_ego['labels'])

            if 'video' in instance_exo and instance_exo['video'] is not None:
                features.append(torch.tensor(instance_exo['video']))
            if 'video' in instance_ego and instance_ego['video'] is not None:
                ego_features.append(torch.tensor(instance_ego['video']))
        
        input_ids = input_ids + ego_input_ids
        labels = labels + ego_labels
        ...
        # if ego_features:
        #     batch['ego_video_spatio_temporal_features'] = torch.stack(ego_features)
...
def train():
    ...
    print("#### loaded ego projection with llava projection weights ######")
    mm_projector_state_dict = model.get_model().mm_projector.state_dict()

    # Load the state dictionary into 'mm_projector_ego'
    model.get_model().mm_projector_ego.load_state_dict(mm_projector_state_dict)
    ...
    if model_args.tune_mm_mlp_adapter:
        ...
        for p in model.get_model().mm_projector_ego.parameters():
            p.requires_grad = True    
