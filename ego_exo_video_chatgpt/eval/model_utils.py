...
from ego_exo_video_chatgpt.model import VideoChatGPTLlamaForCausalLM
from ego_exo_video_chatgpt.utils import disable_torch_init
...
def load_weights(model, projection_path):
    mm_projector_weights = torch.load(projection_path, map_location='cpu')
    embed_tokens = {
    ...
    model.model.embed_tokens.load_state_dict(embed_tokens)
    model.model.mm_projector.load_state_dict(mm_projector_exo_weights)
    status = model.model.mm_projector_ego.load_state_dict(mm_projector_ego_weights)
    return status, model
...
def initialize_model(model_name, projection_path=None):
    ...
    if projection_path:
        ...
        # status, model = load_weights(model, projection_path)
