...
def ego_exo_video_chatgpt_infer(video_frames, question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len):
    """
    ...
    if model.get_model().vision_config.use_vid_start_end:
        qs = question + '\n' + DEFAULT_VID_START_TOKEN + DEFAULT_VIDEO_PATCH_TOKEN * video_token_len + DEFAULT_VID_END_TOKEN + DEFAULT_EGO_START_TOKEN + DEFAULT_EGO_PATCH_TOKEN * video_token_len + DEFAULT_EGO_END_TOKEN 
    else:
        qs = question + '\n' + DEFAULT_VIDEO_PATCH_TOKEN * video_token_len + DEFAULT_EGO_PATCH_TOKEN * video_token_len
    ...
    # Move inputs to GPU
    # print(image_forward_outs.device , frame_features.device)
    ...
    with torch.inference_mode():
        output_ids = model.generate(
            ...
            ego_features=video_spatio_temporal_features.unsqueeze(0),
            do_sample=True,
            temperature=0.2,
            max_new_tokens=1024,
...
def video_chatgpt_infer(video_frames, question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len,is_exo=True):
    """
    Run inference using the Video-ChatGPT model.
    ...
    if is_exo:
        exo_features = video_spatio_temporal_features.unsqueeze(0)
        ego_features = None
    else:
        exo_features = None
        ego_features = video_spatio_temporal_features.unsqueeze(0)  
    ...
    with torch.inference_mode():
        output_ids = model.generate(
            input_ids,
            video_spatio_temporal_features = exo_features,
