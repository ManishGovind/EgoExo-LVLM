...
from ego_exo_video_chatgpt.train.llava_trainer import VideoChatGPTTrainer
from ego_exo_video_chatgpt import video_conversation as conversation_lib
from ego_exo_video_chatgpt.model import *
import torch.distributed as dist
from ego_exo_video_chatgpt.constants import *
import pickle
...
class DataArguments:
    ...
    ego_video_folder: Optional[str] = field(default=None)  # Added this line
...
def preprocess_multimodal(
    ...
    # ego_video_token_len = cur_token_len
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'video' in sources[0]:
            ...
            ego_video_file = sources[0].get('ego_video', '')
            ego_video_folder = self.multimodal_cfg.get('ego_video_folder')
            
            with open(f"{ego_video_folder}/{ego_video_file}", "rb") as f:
                ego_video_features = pickle.load(f)  
        ...
        if 'video' in self.list_data_dict[i]:
            ...
            data_dict["ego_video"] = ego_video_features
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'video' in instances[0]:
            ...
            # print(len(features))
            ego_video_features = [torch.tensor(instance['ego_video']) for instance in instances]
            ...
            if all(x is not None and x.shape == ego_video_features[0].shape for x in ego_video_features):
                batch['ego_features'] = torch.stack(ego_video_features)
            else:
                batch['ego_features'] = ego_video_features    
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = dataset_cls(tokenizer=tokenizer,
                                ...
                                multimodal_cfg=dict(
                                    ...
                                    ego_video_folder=data_args.ego_video_folder,  # add ego video features
...
def train():
    ...
    print("#### loaded ego projection with llava proection weights ######")
