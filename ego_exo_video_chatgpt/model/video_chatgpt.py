...
class VisionConfig:
    def __init__(self):
        ...
        self.ego_start_token = None
        self.ego_end_token = None
        self.ego_patch_token = None
...
class VideoChatGPTLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig, mm_vision_tower=None, mm_hidden_size=None):  # TODO: Remove unused params
        ...
        if hasattr(config, "use_mm_proj"):
            ...
            self.mm_projector_ego = nn.Linear(config.mm_hidden_size, config.hidden_size)
    ...
    def initialize_vision_modules(self, pretrain_mm_mlp_adapter=None, tune_mm_mlp_adapter=False):
        ...
        if not hasattr(self, 'mm_projector'):
            ...
            self.mm_projector_ego = nn.Linear(vision_config.hidden_size, self.config.hidden_size)

        if pretrain_mm_mlp_adapter is not None:
            mm_projector_weights = torch.load(pretrain_mm_mlp_adapter, map_location='cpu')
            # self.mm_projector.load_state_dict({k.split('.')[-1]: v for k, v in mm_projector_weights.items()})
            # self.mm_projector_ego.load_state_dict({k.split('.')[-1]: v for k, v in mm_projector_weights.items()})
            embed_tokens = {
                "weight": mm_projector_weights["model.embed_tokens.weight"]
            }
            mm_projector_exo_weights = {
                "weight": mm_projector_weights["model.mm_projector.weight"],
                "bias": mm_projector_weights["model.mm_projector.bias"],
            }
            mm_projector_ego_weights = {
                "weight": mm_projector_weights["model.mm_projector_ego.weight"],
                "bias": mm_projector_weights["model.mm_projector_ego.bias"],
            }
            #load weights individually otherwise there will be mismatch error
            self.model.model.embed_tokens.load_state_dict(embed_tokens)
            self.model.model.mm_projector.load_state_dict(mm_projector_exo_weights)
            self.model.model.mm_projector_ego.load_state_dict(mm_projector_ego_weights)
    ...
    def forward(
        ...
        if video_spatio_temporal_features is not None and ego_features is not None : 
            # print("train both projections")
            if (input_ids.shape[1] != 1 or self.training) and video_spatio_temporal_features is not None and ego_features is not None :
                # print("both projection")
                ...
                # ego features extracted same as exo video features 
                ego_video_features = self.mm_projector_ego(ego_features)
                dummy_ego_video_features = torch.zeros(ego_video_features.shape[1], 1024, device=inputs_embeds.device,
                                                   dtype=inputs_embeds.dtype)
                dummy_ego_video_features = self.mm_projector_ego(dummy_ego_video_features)
                new_input_embeds = []
                cur_video_idx = 0
                ...
                for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds):
                    ...
                    if self.vision_config.use_vid_start_end:
                        ...
                        for video_start_token_pos in video_start_tokens:
                            ...
                            cur_ego_video_features = ego_video_features[cur_video_idx].to(device=cur_input_embeds.device)
                            num_patches = cur_video_features.shape[0]
                            num_ego_patches = cur_ego_video_features.shape[0]
                            ...
                            if orig_embeds_params is not None:
                                 # ! Get ego video embeddings relative to  exo video embedding
                                # print("ego-exo-dual projection") 
                                ego_start_token_idx = video_start_token_pos + num_patches + 2
                                ego_end_token_idx = ego_start_token_idx + num_ego_patches + 1
                                ...
                                cur_new_input_embeds = torch.cat((cur_input_embeds[:video_start_token_pos].detach(),
                                                                  ...
                                                                  cur_input_embeds[video_start_token_pos + num_patches
                                                                                      ...
                                                                                      + num_patches + 2],
                                                                  cur_input_embeds[ego_start_token_idx:ego_start_token_idx + 1],  # ego start token 
                                                                  cur_ego_video_features, # the ego video  features
                                                                  cur_input_embeds[ego_end_token_idx:ego_end_token_idx + 1],
                                                                  cur_input_embeds[
                                                                   ego_end_token_idx + 1:].detach()), # everything after ego
                                                                    dim=0)
                            else:
                                ego_start_token_idx = video_start_token_pos + num_patches + 2
                                ego_end_token_idx = ego_start_token_idx + num_ego_patches + 1
                                cur_new_input_embeds = torch.cat((cur_input_embeds[:video_start_token_pos],
                                                                  cur_input_embeds[video_start_token_pos:video_start_token_pos + 1],
                                                                  cur_video_features,
                                                                  cur_input_embeds[video_start_token_pos + num_patches + 1:ego_start_token_idx],
                                                                  cur_input_embeds[ego_start_token_idx:ego_start_token_idx + 1],
                                                                  cur_ego_video_features,
                                                                  cur_input_embeds[ego_end_token_idx:ego_end_token_idx + 1],
                                                                  cur_input_embeds[ego_end_token_idx + 1:]), dim=0)
                            cur_video_idx += 1  
        ...
        #only one projection is used 
        else :
            if (input_ids.shape[1] != 1 or self.training) and (video_spatio_temporal_features is not None or ego_features is not None) :
                if  video_spatio_temporal_features is not None :
                    print("processing exo projection")
                    video_features = self.mm_projector(video_spatio_temporal_features)
                    dummy_video_features = torch.zeros(video_features.shape[1], 1024, device=inputs_embeds.device,
                                                    dtype=inputs_embeds.dtype)
                    dummy_video_features = self.mm_projector(dummy_video_features)
                elif ego_features is not None :
                    print("processing ego projection")
                    video_features = self.mm_projector_ego(ego_features)
                    dummy_video_features = torch.zeros(video_features.shape[1], 1024, device=inputs_embeds.device,
                                                    dtype=inputs_embeds.dtype)
                    dummy_video_features = self.mm_projector_ego(dummy_video_features)
        ...
        model_inputs.update(
            {
                ...
                "ego_features": kwargs.get("ego_features", None)
            }
        )
        return model_inputs
