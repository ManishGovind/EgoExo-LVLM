...
def parse_args():
    ...
    parser.add_argument("--ego_exo", type=bool, required=True, default=False)
    parser.add_argument("--dual_projection", type=str, required=False, default="unsync")
...
def run_inference(process_id, args):
    ...
    for i, sample in iterator:
        ...
        if os.path.exists(video_path):
            ...
            try:
                if args.ego_exo :
                    if args.dual_projection == "both":
                        prediction = ego_exo_video_chatgpt_infer(video_frames, full_question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len)
                    else:
                        prediction = video_chatgpt_infer(video_frames, full_question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len,False)     
                else:
                    prediction = video_chatgpt_infer(video_frames, full_question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len, max_new_tokens=args.max_new_tokens)
                # print('================Question================')
                # print(full_question)
                # print('=================Answer=================')
                print(prediction)
...
if __name__ == "__main__":
    ...
    if args.ego_exo:
        if args.dual_projection != 'unsync' :
            from ego_exo_video_chatgpt.eval.model_utils import initialize_model, load_video
            from ego_exo_video_chatgpt.inference import ego_exo_video_chatgpt_infer, video_chatgpt_infer 
        else:
            print("unsync")
